...
public class TwoPlayerScreen implements Screen, InputProcessor {
	...
	private int ballSpeedIncrement = 4;

	public TwoPlayerScreen(ThePingPong game, float ballSpeed, float lrSpeed) {
		this.game = game;
	...
	public void render(float delta) {
		...
		game.lrFont.draw(game.batch, "p:" + lrPoints, 25, 48);
		...
		if (gameRunning) {
			...
			if(rrPoints > 4){
				game.font.draw(game.batch, "Blue Won!!!", 16, 18);
			}else{			
			game.font.draw(game.batch, "Red missed!!!", 12, 18);
			}
		} 
		else if (rrMiss) {
			if(lrPoints > 4){
				game.font.draw(game.batch, "Red Won!!!", 18, 18);
			}else{			
			game.font.draw(game.batch, "Blue missed!!!", 10, 18);
			}
		}
	...
	private void handleInput() {
			for (int i = 0; i < 5; i++) {
				if (Gdx.input.isTouched(i)) {
					...
					if (touchVect.x < 30) {
						if (touchVect.y <= game.redRacket.getHeight() / 2) {
							lrY = 0;
						}
						else if(touchVect.y >= game.SCREEN_HEIGHT - (game.redRacket.getHeight() / 2)){
							lrY = game.SCREEN_HEIGHT - game.redRacket.getHeight();
						}
						else {
							lrY = touchVect.y - (game.redRacket.getHeight() / 2);
						}
					}
					if (touchVect.x > 50) {
						...
						else if(touchVect.y >= game.SCREEN_HEIGHT - (game.blueRacket.getHeight() / 2)){
