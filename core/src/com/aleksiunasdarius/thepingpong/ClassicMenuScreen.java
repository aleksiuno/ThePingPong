...
public class ClassicMenuScreen implements Screen, InputProcessor {
	...
	public ClassicMenuScreen(ThePingPong game) {
	...
	public void render(float delta) {
		...
		game.font.draw(game.batch, "difficulty", BUTTON_COORD_X + 5, TITLE_COORD_Y + FONT_INCREMENT);

		game.font.draw(game.batch, "easy", BUTTON_COORD_X + 20, EASY_BUTTON_COORD_Y + FONT_INCREMENT);
		game.font.draw(game.batch, "medium", BUTTON_COORD_X + 16, MEDIUM_BUTTON_COORD_Y + FONT_INCREMENT);
		game.font.draw(game.batch, "hard", BUTTON_COORD_X + 20, HARD_BUTTON_COORD_Y + FONT_INCREMENT);
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if (touchVect.x > BUTTON_COORD_X && touchVect.x < BUTTON_COORD_X + game.buttonTxt.getWidth()){
			if(touchVect.y > EASY_BUTTON_COORD_Y && touchVect.y < EASY_BUTTON_COORD_Y + game.buttonTxt.getHeight()){
				game.setScreen(new OnePlayerScreen(game, 40, 9));
				dispose();
			}
			if(touchVect.y > MEDIUM_BUTTON_COORD_Y && touchVect.y < MEDIUM_BUTTON_COORD_Y + game.buttonTxt.getHeight()){
				game.setScreen(new OnePlayerScreen(game, 80, 25));
				dispose();
			}
			if(touchVect.y > HARD_BUTTON_COORD_Y && touchVect.y < HARD_BUTTON_COORD_Y + game.buttonTxt.getHeight()){
				game.setScreen(new OnePlayerScreen(game, 100, 37));
				dispose();
			}
		}
		return false;
